...
(function(){
  ...
  /* Maps each animal's display name (BANK keys) to its lesson id, so a quiz
     question can be narrated from the matching pre-recorded clip in audio/ —
     same clip-per-question narration pattern as the badger quiz, rather than
     the earlier silent voice(detail) stub nothing was listening to. Badgers
     is omitted: the real badger quiz (with its own 10-question pool) already
     narrates itself, and this module never builds a tile for it. */
  var NAME_TO_ID={"Foxes":"fox","Bats":"bat","Hedgehogs":"hedgehog","Barn Owls":"owl","Otters":"otter","Pine Martens":"marten","Stoats":"stoat","Frogs":"frog","Moths":"moth","Grey Seals":"seal","Puffins":"puffin","Dolphins":"dolphin","Basking Sharks":"shark"};
  var GENERIC_DONE_CLIP="audio/qz_generic_done.mp3";
  var quizAudio=null;
  function quizPlayer(){
    if(!quizAudio)quizAudio=new Audio();
    return quizAudio;
  }
  ...
  function voice(detail){
    ...
    var id=NAME_TO_ID[detail.animal];
    if(detail.type==="question"&&id!=null&&typeof detail.questionIndex==="number"){
      speakClip("audio/qz_"+id+"_q"+(detail.questionIndex+1)+".mp3");
