/* Cub Quest — Badger Dash post-quiz presentation and narration. */
(function(){
  "use strict";
  ...
  if(!launch || !exits) return;

  /* Keep Badger Dash only inside the completed Badger quiz panel. */
  if(launch.parentNode!==exits){
  ...
  style.textContent=`
    ...
    @media(max-width:520px){#badgerGameBtn.badger-game-tile{min-height:104px;padding:13px 14px}.badger-tile-art{width:60px;height:60px}.badger-tile-copy b{font-size:1.2rem}.badger-tile-copy small{font-size:.82rem}}
  `;
  ...
  /* Narrate the game goal as soon as the child opens the game. */
  ...
  /* Each fact popup is read aloud automatically. */
